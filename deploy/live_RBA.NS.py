...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_RBA.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_RBA.NS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_RBA.NS.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_RBA.NS.json"
