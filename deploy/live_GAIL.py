...
Win Rate: 58.00% (estimated based on PSU gas sector)
Position: ₹7000 | Stop Loss: 0.8% | Target: 4.0x | Daily Loss Cap: 0.3%
Created: 2026-03-22 - New script for GAIL
"""
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_GAIL.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_GAIL.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Win Rate: 58.00%% ===", SYMBOL, STRATEGY)
