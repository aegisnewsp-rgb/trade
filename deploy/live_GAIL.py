...
Enhanced: 2026-03-23 - v8 LOWWR: added MACD + Bollinger Band + Trend MA filters
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_GAIL.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_GAIL")

SYMBOL         = "GAIL.NS"
ENERGY_INDEX   = "^NSEENERGY"  # NIFTY ENERGY index for sector confirmation
...
def fetch_energy_index_data(days: int = 30, retries: int = 3) -> list | None:
    for attempt in range(retries):
        try:
            df = yf.Ticker(ENERGY_INDEX).history(period=f"{days}d")
            if df.empty:
                raise ValueError("Empty energy index dataframe")
            ohlcv = [
                {"date": str(idx.date()), "open": float(r["Open"]), "high": float(r["High"]),
                 "low": float(r["Low"]), "close": float(r["Close"]), "volume": int(r["Volume"])}
                for idx, r in df.iterrows()
            ]
            log.info("Fetched %d candles for energy index", len(ohlcv))
            return ohlcv
        except Exception as e:
            log.warning("Attempt %d/%d failed fetching energy index: %s", attempt + 1, retries, e)
            time.sleep(2 ** attempt)
    log.warning("Energy index fetch failed, proceeding without sector confirmation")
    return None
...
def calculate_energy_index_signal(energy_data: list) -> str:
    if not energy_data or len(energy_data) < 5:
        return "NEUTRAL"
    closes = [bar["close"] for bar in energy_data[-10:]]
    if len(closes) < 5:
        return "NEUTRAL"
    ma5 = sum(closes[-5:]) / 5
...
def vwap_signal(ohlcv: list, params: dict, energy_data: list = None) -> tuple:
    ...
    energy_signal = calculate_energy_index_signal(energy_data) if energy_data else "NEUTRAL"
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: GAIL.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")

    ohlcv = fetch_recent_data(days=60)
    energy_data = fetch_energy_index_data(days=30)

    if not ohlcv:
        print("No data fetched")
        return

    signal, price, atr, rsi = vwap_signal(ohlcv, PARAMS, energy_data)
