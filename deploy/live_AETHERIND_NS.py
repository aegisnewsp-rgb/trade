#!/usr/bin/env python3
"""
Live Trading Script - AETHERIND.NS
Strategy: VWAP Momentum (Volume Weighted Average Price with RSI confirmation)
Source: Economic Times Market Trading Guide (+9% short-term potential)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_AETHERIND_NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_AETHERIND_NS")
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_AETHERIND_NS.json"
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_AETHERIND_NS.json"
