#!/usr/bin/env python3
"""
Live Trading Script - SUNDARAM.BO.py
Strategy: MEAN_REVERSION (RSI-only + VWAP bounce) - v9 LOWWR
Win Rate: 10-13% -> Target 50%+ (MEAN_REVERSION v9 - simpler filter set)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v9 MEAN_REVERSION: pivoted from v8 (multi-filter too restrictive)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SUNDARAM_BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SUNDARAM_BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "SUNDARAM.BO"
STRATEGY = "MEAN_REVERSION_RSI_V9"  # SUNDARAM.py
TARGET_1_MULT = 1.5
TARGET_2_MULT = 3.0
TARGET_3_MULT = 5.0
STOP_LOSS_PCT  = 0.006
