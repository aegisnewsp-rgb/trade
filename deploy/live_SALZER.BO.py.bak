...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SALZER.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SALZER")
...
def log_signal(signal, price, atr):
    log_file = LOG_DIR / "signals_SALZER.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit():
    cap_file = LOG_DIR / "daily_pnl_SALZER.json"
