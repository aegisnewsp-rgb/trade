...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SAMB.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SAMB")
...
def log_signal(signal, price, atr):
    log_file = LOG_DIR / "signals_SAMB.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit():
    cap_file = LOG_DIR / "daily_pnl_SAMB.json"
