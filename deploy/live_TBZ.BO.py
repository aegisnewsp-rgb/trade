#!/usr/bin/env python3
"""
Live Trading Script - TBZ.BO
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8)
Win Rate: TBZ.BO (low base) -> Target 55%+ (v8 multi-filter upgrade)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_TBZ_BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_TBZ_BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "TBZ.BO"
STRATEGY       = "VWAP_RSI_MACD_VOL_BB_v8_LOWWR"  # TBZ.BO
POSITION       = 7000
