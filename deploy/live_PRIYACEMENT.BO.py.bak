...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_PRIYACEMENT.BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_PRIYACEMENT.BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "PRIYACEMENT.BO"
STRATEGY       = "VWAP"
POSITION       = 7000
STOP_LOSS_PCT  = 0.008
TARGET_MULT    = 4.0
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_PRIYACEMENT.BO.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_PRIYACEMENT.BO.json"
