...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_BHARTIARTL.json"
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_BHARTIARTL.json"
...
def main():
    ...
    if signal != "HOLD" and GROWW_API_KEY and GROWW_API_SECRET:
        ...
        if result:
            ...
            log.warning(
                "⚠ Groww order could not be placed – signal still printed/logged."
            )
    elif signal != "HOLD":
        log.info(
