...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_PETPLT.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_PETPLT")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_PETPLT.json"
    entries = []
    if log_file.exists():
        try:
            ...
            entries = []
    entries.append({
        "timestamp": ist_now().isoformat(), "symbol": SYMBOL,
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_PETPLT.json"
