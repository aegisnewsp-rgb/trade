#!/usr/bin/env python3
"""
Live Trading Script - INDHOTEL.NS
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8 LOWWR)
Win Rate: 58.62% -> Target 65%+ (v8 LOWWR: full multi-filter upgrade from basic VWAP)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: upgraded from basic VWAP to multi-filter strategy
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_INDHOTEL.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_INDHOTEL")
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: INDHOTEL.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")
