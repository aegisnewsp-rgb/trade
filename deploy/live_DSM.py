#!/usr/bin/env python3
"""
Live Trading Script - DSM
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8)
Win Rate: DSM (low base) -> Target 55%+ (v8 multi-filter upgrade)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_DSM.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_DSM")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "DSM"
STRATEGY       = "VWAP_RSI_MACD_VOL_BB_v8_LOWWR"  # DSM
POSITION       = 7000
