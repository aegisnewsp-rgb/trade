#!/usr/bin/env python3
"""
Live Trading Script - SOL.active.BO
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SOL.active.BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SOL.active.BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "SOL.active.BO"
STRATEGY       = "VWAP"
POSITION       = 7000
STOP_LOSS_PCT  = 0.008
TARGET_MULT    = 4.0
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_SOL.active.BO.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_SOL.active.BO.json"
