#!/usr/bin/env python3
"""
Live Trading Script - HAPPSTMIND.NS (Happiest Minds Technologies)
Strategy: VWAP + RSI + Volume Confirmation (v4 enhanced)
Added: 2026-03-22 | Top NSE gainer +10.55%, IT sector momentum, mid-cap growth play
...
Note: Extended single-day gain (+10.55%) — watch for mean reversion on open.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_HAPPSTMIND.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_HAPPSTMIND")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_HAPPSTMIND.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_HAPPSTMIND.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Top Gainer +10.55%% | IT Sector ===",
             SYMBOL, STRATEGY)
    log.warning("⚠️  Extended single-day gain (+10.55%%) — watch for mean reversion on open")
