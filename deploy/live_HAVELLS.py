#!/usr/bin/env python3
"""
Live Trading Script - HAVELLS.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_HAVELLS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_HAVELLS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_HAVELLS.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_HAVELLS.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Win Rate: 62.07%% ===", SYMBOL, STRATEGY)
