...
def calculate_psar(ohlcv: list, af_start: float = 0.02, af_inc: float = 0.02, af_max: float = 0.2) -> list:
    highs    = [b["high"]  for b in ohlcv]
    lows     = [b["low"]   for b in ohlcv]
    psar     = [0.0] * len(ohlcv)
    trend    = [1] * len(ohlcv)   # 1=up, -1=down
    ep        = [highs[0]] * len(ohlcv)
    af        = [af_start] * len(ohlcv)
    ...
    for i in range(1, len(ohlcv)):
        ...
        if prev_trend == 1:
            ...
            if lows[i] < psar[i]:
                ...
                if highs[i] > prev_ep:
                    psar[i] = psar[i - 1] + prev_af * (highs[i] - psar[i - 1])
        else:
            ...
            if highs[i] > psar[i]:
                ...
                af[i]    = min(prev_af + af_inc, af_max)
                if lows[i] < prev_ep:
                    psar[i] = psar[i - 1] - prev_af * (psar[i - 1] - lows[i])
    return psar, trend
...
def main():
    ...
    if signal == "BUY":
        ...
        target_prc = round(price + TARGET_MULT * atr,