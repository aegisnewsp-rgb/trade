...
Win Rate: 57.92% -> Target 65%+ (v8 LOWWR: full multi-filter upgrade from PARABOLIC_SAR)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: replaced PARABOLIC_SAR with multi-filter VWAP strategy
"""
...
SYMBOL         = "ALKEM.NS"
PHARMA_INDEX   = "^NSEPHARMA"  # NIFTY PHARMA index for sector confirmation
...
def fetch_pharma_index_data(days: int = 30, retries: int = 3) -> list | None:
    for attempt in range(retries):
        try:
            df = yf.Ticker(PHARMA_INDEX).history(period=f"{days}d")
            if df.empty:
                raise ValueError("Empty pharma index dataframe")
            ohlcv = [
                {"date": str(idx.date()), "open": float(r["Open"]), "high": float(r["High"]),
                 "low": float(r["Low"]), "close": float(r["Close"]), "volume": int(r["Volume"])}
                for idx, r in df.iterrows()
            ]
            log.info("Fetched %d candles for pharma index", len(ohlcv))
            return ohlcv
        except Exception as e:
            log.warning("Attempt %d/%d failed fetching pharma index: %s", attempt + 1, retries, e)
            time.sleep(2 ** attempt)
    log.warning("Pharma index fetch failed, proceeding without sector confirmation")
    return None
...
def calculate_pharma_index_signal(pharma_data: list) -> str:
    if not pharma_data or len(pharma_data) < 5:
        return "NEUTRAL"
    closes = [bar["close"] for bar in pharma_data[-10:]]
    if len(closes) < 5:
        return "NEUTRAL"
    ma5 = sum(closes[-5:]) / 5
...
def vwap_signal(ohlcv: list, params: dict, pharma_data: list = None) -> tuple:
    ...
    pharma_signal = calculate_pharma_index_signal(pharma_data) if pharma_data else "NEUTRAL"
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: ALKEM.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")

    ohlcv = fetch_recent_data(days=60)
    pharma_data = fetch_pharma_index_data(days=30)

    if not ohlcv:
        print("No data fetched")
        return

    signal, price, atr, rsi = vwap_signal(ohlcv, PARAMS, pharma_data)
