#!/usr/bin/env python3
"""
Live Trading Script - HGINFRA.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_HGINFRA.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_HGINFRA")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_HGINFRA.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_HGINFRA.json"
