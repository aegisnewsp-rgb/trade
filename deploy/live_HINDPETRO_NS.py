...
LOG_DIR = Path("/tmp")
STATE_FILE = Path("/home/node/workspace/trade-project/deploy/state_HINDPETRO.json")

def setup_logging():
    log_file = LOG_DIR / f"trades_HINDPETRO.log"
    logging.basicConfig(
        level=logging.INFO,
...
def main():
    ...
    try:
        ...
        if len(d) < 30:
            print("HINDPETRO: No data")
            return
        closes = d['Close'].tolist()
        print(f"HINDPETRO: {len(closes)} candles, last price {closes[-1]:.2f}")
    except Exception as e:
        print(f"HINDPETRO: Error - {e}")
