...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_KIRTID.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_KIRTID")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_KIRTID.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_KIRTID.json"
