#!/usr/bin/env python3
"""
Live Trading Script - WSTCSTPAPR.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_WSTCSTPAPR.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_WSTCSTPAPR")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_WSTCSTPAPR.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_WSTCSTPAPR.json"
