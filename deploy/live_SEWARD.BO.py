...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SEWARD.BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SEWARD.BO")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_SEWARD.BO.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_SEWARD.BO.json"
