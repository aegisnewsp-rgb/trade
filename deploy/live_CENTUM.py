#!/usr/bin/env python3
"""
Live Trading Script - CENTUM.NS (Centum Electronics)
Strategy: VWAP (Volume Weighted Average Price)
Source: Economic Times Market Trading Guide (+9% short-term target, Mar 22 2026)
Key Stats: Trading near life-highs | Breakout above ₹2,800 | Higher-high/higher-low structure
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_CENTUM.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_CENTUM")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_CENTUM.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_CENTUM.json"
...
def main():
    ...
    log.info("Source: Economic Times Market Trading Guide (+9%% short-term, Mar 22 2026)")
    log.info("Context: Trading near life-highs | Breakout above ₹2,800 | Higher-high structure")
