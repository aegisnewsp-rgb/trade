#!/usr/bin/env python3
"""
Live Trading Script - TATAMOTORS.BO
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8 LOWWR)
Win Rate: 63.64% -> Target 68%+ (v8 LOWWR: full multi-filter upgrade from basic VWAP)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: upgraded from basic VWAP to multi-filter
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_TATAMOTORS.BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_TATAMOTORS.BO")
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: TATAMOTORS.BO | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")
