#!/usr/bin/env python3
"""
Live Trading Script - SIGachi.BO
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SIGachi.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SIGachi")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_SIGachi.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_SIGachi.json"
