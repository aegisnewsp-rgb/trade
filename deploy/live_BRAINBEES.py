...
Win Rate: N/A (new candidate, momentum-based)
Position: ₹7000 | Stop Loss: 0.8% ATR | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Note: Top NSE gainer 2026-03-22 with +19.43% move
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_BRAINBEES.log"),
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_BRAINBEES.json"
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_BRAINBEES.json"
...
def main():
    ...
    log.info("Note: Brainbees Solutions - Top NSE gainer 2026-03-22 (+19.43%%)")
    while is_pre_market():
