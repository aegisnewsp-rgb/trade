#!/usr/bin/env python3
"""
Live Trading Script - ESCORTS.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_ESCORTS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_ESCORTS")
...
def vwap_rsi_signal(ohlcv: list, params: dict) -> tuple[str, float, float, float]:
    """
    VWAP + RSI confirmation strategy.
    BUY:  price > VWAP + ATR*mult AND RSI < rsi_overbought (not overbought)
    SELL: price < VWAP - ATR*mult AND RSI > rsi_oversold (not oversold)
    """
    ...
    for i in range(period, len(ohlcv)):
        ...
        # RSI confirmation: for BUY, RSI should be below overbought (not too hot)
        # for SELL, RSI should be above oversold (not too cold)
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_ESCORTS.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_ESCORTS.json"
...
def main():
    ...
    log.info("  STRATEGY : %s (enhanced with RSI confirmation)", STRATEGY)
    log.info("  SIGNAL   : ★ %s ★", signal)
    log.info("  PRICE    : ₹%.2f", price)
    log.info("  QTY      : %d shares (₹%d position)", quantity, POSITION)
    if atr > 0:
        log.info("  ATR      : %.4f", atr)
        log.info("  RSI      : %.1f (overbought=%d, oversold=%d)", rsi, PARAMS["rsi_overbought"], PARAMS["rsi_oversold"])
