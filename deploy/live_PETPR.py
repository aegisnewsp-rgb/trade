#!/usr/bin/env python3
"""Live Trading Script - PETPR.BO | VWAP | Risk: ₹7000 | Stop: 0.8% ATR | Target: 4.0× ATR"""
...
log=logging.getLogger("live_PETPR")
...
def fetch_recent_data(days=60,retries=3):
    for attempt in range(retries):
        try:
            ...
            ohlcv=[{"date":str(idx.date()),"open":float(row["Open"]),"high":float(row["High"]),"low":float(row["Low"]),"close":float(row["Close"]),"volume":int(row["Volume"])} for idx,row in df.iterrows()]
            log.info("Fetched %d candles for %s",len(ohlcv),SYMBOL); return ohlcv
        except Exception as e:
            log.warning("Attempt %d/%d failed: %s",attempt+1,retries,e); time.sleep(2**attempt)
    log.error("All fetch attempts failed for %s",SYMBOL); return None
...
def vwap_signal(ohlcv,params):
    ...
    vwap_vals=calculate_vwap(ohlcv,period); atr_vals=calculate_atr(ohlcv,period); signals=["HOLD"]*len(ohlcv)
...
def place_groww_order(symbol,signal,quantity,price):
    ...
    url=f"GROWW_API_BASE/orders"; payload={"symbol":symbol,"exchange":"BSE","transaction":"BUY" if signal=="BUY" else "SELL","quantity":quantity,"price":round(price,2),"order_type":"LIMIT","product":"CNC"}
    headers={"Authorization":f"Bearer {GROWW_API_KEY}","X-Api-Secret":GROWW_API_SECRET,"Content-Type":"application/json"}
    for attempt in range(3):
        try:
            resp=requests.post(url,json=payload,headers=headers,timeout=10)
            if resp.status_code in(200,201): log.info("Groww order placed: %s",resp.json()); return resp.json()
            log.warning("Groww API attempt %d: HTTP %d – %s",attempt+1,resp.status_code,resp.text)
        except Exception as e: log.warning("Groww order attempt %d failed: %s",attempt+1,e)
        time.sleep(2**attempt)
    log.error("Groww order failed after 3 retries for %s",symbol); return None
def log_signal(signal,price,atr):
    log_file=LOG_DIR/"signals_PETPR.json"; entries=[]
    if log_file.exists():
...
def daily_loss_limit_hit():
    cap_file=LOG_DIR/"daily_pnl_PETPR.json"; today_str=ist_now().strftime("%Y-%m-%d")
    if cap_file.exists():
        try:
