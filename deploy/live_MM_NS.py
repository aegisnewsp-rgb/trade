...
Win Rate: 16.7% -> Target 55%+ (v9 MEAN_REVERSION - pivoted from v8 multi-filter too restrictive)
...
Preserved: Festival Season + Tractor Sales proxy filters (M&M agricultural focus)
"""
...
# Festival and seasonality (preserved from original)
FESTIVE_MONTHS = [1, 9, 10, 11]
HARVEST_SEASON_MONTHS = [2, 3, 4, 10, 11]
TRACTOR_SALES_ETB_SYMBOL = "TRACTORIND.NS"
...
def apply_festive_and_tractor_filters(signal: str, festive_status: str,
    ...
    """Apply M&M-specific rural/agriculture filters on top of v9 MEAN_REVERSION signal"""
    if signal == "HOLD":
        return signal
    ...
    if festive_status == "OFF_SEASON":
        if signal == "BUY":
            ...
            return "HOLD"
    elif festive_status in ("FESTIVE_PEAK", "HARVEST_ACTIVE"):
        if signal == "SELL":
...
def vwap_signal(ohlcv: list, params: dict) -> tuple:
    ...
    No trend filter, no MACD, no Bollinger Bands (too restrictive for M&M).
    """
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: M&M.NS | Strategy: MEAN_REVERSION v9 (RSI + VWAP bounce)")
    print(f"{'='*60}")
    ...
    print(f"Signal: {filtered_signal} (raw: {signal})")
    print(f"Price:  Rs{price:.2f} | ATR: Rs{atr:.2f} | RSI: {rsi:.1f}")
    print(f"Season: {festive_status} | Tractor Sales: {tractor_sales}")
