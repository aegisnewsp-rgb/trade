...
Research Date: 2026-03-22 | Momentum: +4.91% (5d) | PE: 22x (attractive)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_MM.log"),
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_MM.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_MM.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Momentum: +4.91%% (5d) | PE: 22x ===", SYMBOL, STRATEGY)
