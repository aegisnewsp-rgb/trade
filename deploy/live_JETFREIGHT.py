#!/usr/bin/env python3
"""
Live Trading Script - JETFREIGHT.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_JETFREIGHT.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_JETFREIGHT")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_JETFREIGHT.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_JETFREIGHT.json"
