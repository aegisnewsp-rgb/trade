#!/usr/bin/env python3
"""
Live Trading Script - DELHIVERY.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_DELHIVERY.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_DELHIVERY.NS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_DELHIVERY.NS.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_DELHIVERY.NS.json"
