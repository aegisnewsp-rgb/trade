...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler(LOG_DIR / "live_ARISE.BO.log"), logging.StreamHandler(sys.stdout)])
log = logging.getLogger("live_ARISE.BO")

SYMBOL = "ARISE.BO"; STRATEGY = "VWAP"; POSITION = 7000; STOP_LOSS_PCT = 0.008; TARGET_MULT = 4.0; DAILY_LOSS_CAP = 0.003
...
def log_signal(signal, price, atr):
    log_file = LOG_DIR / "signals_ARISE.BO.json"
...
def daily_loss_limit_hit():
    cap_file = LOG_DIR / "daily_pnl_ARISE.BO.json"; today_str = ist_now().strftime("%Y-%m-%d")
    if cap_file.exists():
        try:
