...
Win Rate: 58.33% -> Target 65%+ (v8 LOWWR: full multi-filter upgrade from VWAP+RSI v2)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: added MACD + Bollinger Band + Trend MA to v2
"""
...
CONSUMER_INDEX = "^NSECONSUMER"  # NIFTY CONSUMER for sector confirmation
...
def fetch_consumer_index_data(days: int = 30, retries: int = 3) -> list | None:
    for attempt in range(retries):
        try:
            df = yf.Ticker(CONSUMER_INDEX).history(period=f"{days}d")
            if df.empty:
                raise ValueError("Empty consumer index dataframe")
            ohlcv = [
                {"date": str(idx.date()), "open": float(r["Open"]), "high": float(r["High"]),
                 "low": float(r["Low"]), "close": float(r["Close"]), "volume": int(r["Volume"])}
                for idx, r in df.iterrows()
            ]
            log.info("Fetched %d candles for consumer index", len(ohlcv))
            return ohlcv
        except Exception as e:
            log.warning("Attempt %d/%d failed fetching consumer index: %s", attempt + 1, retries, e)
            time.sleep(2 ** attempt)
    log.warning("Consumer index fetch failed, proceeding without sector confirmation")
    return None
...
def calculate_consumer_index_signal(consumer_data: list) -> str:
    if not consumer_data or len(consumer_data) < 5:
        return "NEUTRAL"
    closes = [bar["close"] for bar in consumer_data[-10:]]
    if len(closes) < 5:
        return "NEUTRAL"
    ma5 = sum(closes[-5:]) / 5
...
def vwap_signal(ohlcv: list, params: dict, consumer_data: list = None) -> tuple:
    ...
    consumer_signal = calculate_consumer_index_signal(consumer_data) if consumer_data else "NEUTRAL"
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: ASIANPAINT.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")

    ohlcv = fetch_recent_data(days=60)
    consumer_data = fetch_consumer_index_data(days=30)

    if not ohlcv:
        print("No data fetched")
        return

    signal, price, atr, rsi = vwap_signal(ohlcv, PARAMS, consumer_data)
