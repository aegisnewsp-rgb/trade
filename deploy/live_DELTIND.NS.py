#!/usr/bin/env python3
"""
Live Trading Script - DELTIND.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_DELTIND.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_DELTIND.NS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_DELTIND.NS.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_DELTIND.NS.json"
