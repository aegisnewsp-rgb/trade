#!/usr/bin/env python3
"""
Live Trading Script - NAVKARCORP.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_NAVKARCORP.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_NAVKARCORP.NS")
...
def calculate_atr(ohlcv: list, period: int = 14) -> list:
    ...
    for i, bar in enumerate(ohlcv):
        ...
        atr.append(tr if i < period - 1 else (atr[-1] * (period - 1) + tr) / period if i == period - 1 else (atr[-1] * (period - 1) + tr) / period)
...
def place_groww_order(symbol: str, signal: str, quantity: int, price: float) -> dict | None:
    ...
    payload = {"symbol": symbol, "exchange": "NSE", "transaction": "BUY" if signal == "BUY" else "SELL",
    ...
    for attempt in range(3):
        try:
            ...
            log.warning("Groww API attempt %d: HTTP %d", attempt + 1, resp.status_code)
        except Exception as e:
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_NAVKARCORP.NS.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_NAVKARCORP.NS.json"
