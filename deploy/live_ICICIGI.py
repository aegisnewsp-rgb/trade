#!/usr/bin/env python3
"""
Live Trading Script - ICICIGI.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_ICICIGI.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_ICICIGI")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_ICICIGI.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_ICICIGI.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Win Rate: 63.64%% | RSI Filter: %d/%d ===", SYMBOL, STRATEGY, RSI_OVERSOLD, RSI_OVERBOUGHT)
