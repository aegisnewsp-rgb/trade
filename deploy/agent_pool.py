...
def save_status(status):
    status["last_update"] = (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)) + datetime.timedelta(hours=5, minutes=30).isoformat()
...
def enqueue_task(task: dict, priority: int = 5):
    ...
    tasks.append({
        ...
        "priority": priority,
        "added_at": (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)) + datetime.timedelta(hours=5, minutes=30).isoformat(),
        "status": "pending",
    })
...
def get_next_task():
    ...
    for t in tasks:
        if t["status"] == "pending":
            t["status"] = "claimed"
            t["claimed_at"] = (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)) + datetime.timedelta(hours=5, minutes=30).isoformat()
            save_queue(tasks)
            return t
    return None
...
def mark_task_done(task_description: str, status: str = "done"):
    ...
    for t in tasks:
        if t["description"] == task_description and t["status"] == "claimed":
            t["status"] = status
            t["completed_at"] = (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)) + datetime.timedelta(hours=5, minutes=30).isoformat()
            save_queue(tasks)
            return True
    return False
...
"""
    ...
    return {
        ...
        "created_at": (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)) + datetime.timedelta(hours=5, minutes=30).isoformat(),
    }
