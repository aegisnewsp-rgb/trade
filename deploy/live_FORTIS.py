#!/usr/bin/env python3
"""
Live Trading Script - FORTIS.NS
Strategy: VWAP + RSI Confirmation
Win Rate: 59.00% (estimated based on healthcare sector)
Position: ₹7000 | Stop Loss: 0.8% | Target: 4.0x | Daily Loss Cap: 0.3%
Enhanced: 2026-03-22 - Fixed logger bug (was TCS), added RSI filter
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_FORTIS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_FORTIS")
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_FORTIS.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_FORTIS.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Win Rate: 59.00%% ===", SYMBOL, STRATEGY)
