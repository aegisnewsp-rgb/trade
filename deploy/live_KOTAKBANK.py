#!/usr/bin/env python3
"""
Live Trading Script - KOTAKBANK.NS
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8 LOWWR)
Win Rate: N/A -> Target 60%+ (v8 LOWWR: full multi-filter upgrade from basic VWAP_RSI)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: upgraded from basic VWAP_RSI to multi-filter
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_KOTAKBANK.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_KOTAKBANK")
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: KOTAKBANK.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")
