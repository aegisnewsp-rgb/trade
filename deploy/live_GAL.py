#!/usr/bin/env python3
"""
Live Trading Script - GAL
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8)
Win Rate: GAL (low base) -> Target 55%+ (v8 multi-filter upgrade)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_GAL.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_GAL")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "GAL"
STRATEGY       = "VWAP_RSI_MACD_VOL_BB_v8_LOWWR"  # GAL
POSITION       = 7000
