...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_MRF.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_MRF")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_MRF.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_MRF.json"
