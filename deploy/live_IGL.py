...
Win Rate: 60.2% -> Target 65%+ (v8 LOWWR: upgraded from ADX_TREND to multi-filter)
Position: ₹7000 | Stop Loss: 0.6% | Target: 4.0x ATR | Daily Loss Cap: 0.3%
Enhanced: 2026-03-23 - v8 LOWWR: gas price correlation + multi-filter upgrade
Fixed: YFINANCE_AVAILABLE assignment bug
"""
...
# Entry window (energy/CNG sector has specific volume patterns)
...
def check_gas_price_correlation() -> dict:
    ...
    Rising gas prices are FAVORABLE for IGL (pass-through pricing).
    """
    ...
    try:
        ...
        if len(closes) >= 3:
            ...
            # For IGL (gas distribution), rising gas prices are generally FAVORABLE
...
def vwap_signal(ohlcv: list, params: dict, gas_corr: dict = None) -> tuple:
    ...
    gas_favorable = True
    if gas_corr:
        gas_favorable = gas_corr.get("favorable", True)
    ...
    for i in range(start_idx, len(ohlcv)):
        ...
        # Gas price tailwind: if gas BULLISH, strengthen BUY conviction
        if gas_corr and gas_corr.get("trend") == "BULLISH":
            buy_ok = buy_ok  # natural gas tailwind for IGL
        elif gas_corr and gas_corr.get("trend") == "BEARISH":
            buy_ok = buy_ok and (rsi < 28)  # require deeper oversold in bear gas market
        ...
        # If gas prices are rising, block SELL (unfavorable for IGL sell signals)
        if gas_corr and gas_corr.get("trend") == "BULLISH" and gas_corr.get("favorable", True):
            sell_ok = sell_ok and (rsi > 80)  # require deeper overbought
...
def main():
    print(f"\n{'='*60}")
    print(f"Running: IGL.NS | Strategy: VWAP_RSI_MACD_VOL_BB_v8_LOWWR")
    print(f"{'='*60}")
    ...
    signal, price, atr, rsi = vwap_signal(ohlcv, PARAMS, gas_corr)

    print(f"Signal: {signal} | Price: Rs{price:.2f} | ATR: Rs{atr:.2f} | RSI: {rsi:.1f}")
    if gas_corr:
        print(f"Gas: {gas_corr.get('trend','N/A')} | Favorable: {gas_corr.get('favorable','N/A')}")
