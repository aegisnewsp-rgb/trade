#!/usr/bin/env python3
"""
Live Trading Script - PFL.BO
Strategy: VWAP + RSI + MACD + Volume + Trend + Bollinger Band (Enhanced v8)
Win Rate: PFL.BO (low base) -> Target 55%+ (v8 multi-filter upgrade)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_PFL_BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_PFL_BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "PFL.BO"
STRATEGY       = "VWAP_RSI_MACD_VOL_BB_v8_LOWWR"  # PFL.BO
POSITION       = 7000
