#!/usr/bin/env python3
"""
Live Trading Script - SHREECEM.NS
Strategy: MEAN_REVERSION v9 (RSI-only + VWAP bounce) - NO trend/MACD/BB filters
Win Rate: 0% -> Target 50%+ - v9 MEAN_REVERSION (removed trend/MACD/BB filters)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SHREECEM_NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_live_SHREECEM_NS")
...
def calculate_vwap(ohlcv: list, period: int = 14) -> list:
    ...
    for i in range(len(ohlcv)):
        if i < period - 1:
            ...
            tp_sum  = sum((ohlcv[j][1] + ohlcv[j][2] + ohlcv[j][3]) / 3 for j in range(i - period + 1, i + 1))
...
def main():
    ...
    try:
        ...
        signal, price, atr, rsi = sig_result[0], float(sig_result[1]), float(sig_result[2]), float(sig_result[3])
    except Exception as e:
        print(f"Signal error: {e}")
        signal, price, atr = "HOLD", ohlcv_list[-1][3], 0.0
        rsi = 50.0

    print(f"Signal: {signal}")
    print(f"Price:  Rs{price:.2f}")
    print(f"ATR:    Rs{atr:.2f}  RSI: {rsi:.1f}")
