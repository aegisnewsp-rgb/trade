...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_BPCL.log"),
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_BPCL.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_BPCL.json"
...
def main():
    log.info("=== Live Trading: %s | %s | Win Rate: 57.50%% ===", SYMBOL, STRATEGY)
    while is_pre_market():
