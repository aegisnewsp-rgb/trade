#!/usr/bin/env python3
"""
Live Trading Script - FIRSTCRY.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_FIRSTCRY.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_FIRSTCRY.NS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_FIRSTCRY.NS.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_FIRSTCRY.NS.json"
