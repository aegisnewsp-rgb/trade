#!/usr/bin/env python3
"""
Groww Strategy: LUPIN.NS — BUY on VWAP breakout
Market Regime: DOWNTREND (strict size, defensive pharma)
Score: 42.5/100 — highest score

Entry: Price > VWAP + 0.5% with RSI 40-60
Stop:  -0.8%
Target: +2.4% (3:1 R:R)
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"LUPIN.NS  |  {ist.strftime('%H:%M IST')}  |  DOWNTREND REGIME")
    print(f"{'='*60}")
    tk = yf.Ticker(SYMBOL)
    ...
    cond_vwap  = price > vwap * 1.003   # tightened from 1.005 — LUPIN sits very close to VWAP
    cond_rsi   = 40 < rsi < 60
    cond_trend = price > ema20 * 0.995
    print(f"Price: Rs{price:.2f} | VWAP: Rs{vwap:.2f} {'✓' if cond_vwap else '✗'} | EMA20: Rs{ema20:.2f} | RSI: {rsi:.1f} {'✓' if cond_rsi else '✗'}")
    if not (cond_vwap and cond_rsi and cond_trend):
        print("Conditions not met — HOLD"); return
    sl = round(price * (1 - STOP_PCT), 2); target = round(price * (1 + TARGET_PCT), 2)
    qty = max(1, int(POSITION / price))
    print(f"\nSignal: BUY | Entry: Rs{price:.2f} | Stop: Rs{sl:.2f} | Target: Rs{target:.2f} | Qty: {qty}")
    if is_configured():
        token = get_token()
        if token: place_bo(token, EXCHANGE, SYMBOL, "BUY", qty, target, sl)
    else:
