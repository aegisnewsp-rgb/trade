#!/usr/bin/env python3
"""
Live Trading Script - LTES.NS (L&T Technology Services Ltd)
...
Note: IT momentum play — Nifty IT surged +2.17% on Mar 20; L&T Tech ER&D focus
      differentiates from generic IT. Not yet in deploy/ — new addition.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_LTES.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_LTES")
...
def vwap_momentum_signal(ohlcv: list, params: dict) -> tuple[str, float, float]:
    ...
    for i in range(period, len(ohlcv)):
        ...
        # BUY: price above VWAP + ATR band AND RSI confirming momentum (not overbought)
        if price > v + a * atr_mult and r < rsi_ob:
            signals[i] = "BUY"
        # SELL: price below VWAP - ATR band AND RSI confirming weakness (not oversold)
...
def log_signal(signal: str, price: float, atr: float, rsi: float):
    log_file = LOG_DIR / "signals_LTES.json"
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_LTES.json"
...
def main():
    log.info(
        "=== Live Trading Script: %s | %s | Position: ₹%d ===",
        SYMBOL,
        STRATEGY,
        POSITION,
    )
