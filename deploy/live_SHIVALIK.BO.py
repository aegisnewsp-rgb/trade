...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_SHIVALIK_BO.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_SHIVALIK_BO")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "SHIVALIK.BO"
STRATEGY = "MEAN_REVERSION_RSI_V9C"  # SHIVALIK.BO
POSITION       = 5000
...
def vwap_signal(ohlcv: list, params: dict) -> tuple[str, float, float, float]:
    ...
    for i in range(start_idx, len(ohlcv)):
        ...
        # Mean reversion: oversold + RSI crossover + price near/at VWAP
        ...
        # Overbought: overbought + RSI crossover + price near/at VWAP resistance
