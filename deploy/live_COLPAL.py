...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_COLPAL.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_COLPAL")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_COLPAL.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_COLPAL.json"
...
def main():
    log.info("=== Live Trading Script: %s | %s | Win Rate: 62.96%% ===", SYMBOL, STRATEGY)
