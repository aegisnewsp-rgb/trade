#!/bin/bash
# Worker BRAVO-3: Enhance BANKINDIA with multi-timeframe confirmation
cd /home/node/workspace/trade-project

SCRIPT="deploy/live_BANKINDIA.py"

# Enhancement: Multi-timeframe confirmation (daily + hourly VWAP alignment)
...
python3 -m py_compile "$SCRIPT" && echo "BRAVO-3: BANKINDIA compile OK" || echo "BRAVO-3: BANKINDIA compile FAILED"

cd /home/node/workspace/trade-project
git add deploy/live_BANKINDIA.py
git commit -m "BRAVO-3: Enhance BANKINDIA with multi-timeframe VWAP confirmation" 2>&1 || echo "BRAVO-3: No changes to commit"

echo "BRAVO-3 WORKER COMPLETE"