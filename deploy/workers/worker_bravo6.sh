#!/bin/bash
# Worker BRAVO-6: Enhance BECTORFOOD VWAP strategy
cd /home/node/workspace/trade-project

SCRIPT="deploy/live_BECTORFOOD.py"

# Read original and enhance with volume confirmation
cat > "$SCRIPT.enhanced" << 'ENHANCED_SCRIPT'
#!/usr/bin/env python3
"""
Live Trading Script - BECTORFOOD.NS
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler(LOG_DIR / "live_BECTORFOOD.log"), logging.StreamHandler(sys.stdout)])
log = logging.getLogger("live_BECTORFOOD")
...
def main():
    log.info("=== %s | %s (Volume Confirmed) ===", SYMBOL, STRATEGY)
    ...
    log_signal(sig, price, atr)

def log_signal(sig, price, atr):
    f = LOG_DIR/"signals_BECTORFOOD.json"
...
python3 -m py_compile "$SCRIPT" && echo "BRAVO-6: BECTORFOOD OK" || echo "BRAVO-6: FAILED"
cd /home/node/workspace/trade-project
git add deploy/live_BECTORFOOD.py && git commit -m "BRAVO-6: Enhance BECTORFOOD with volume confirmation" 2>&1 || echo "No changes"
echo "BRAVO-6 COMPLETE"