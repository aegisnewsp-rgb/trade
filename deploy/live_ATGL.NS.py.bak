...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_ATGL.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_ATGL.NS")
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_ATGL.NS.json"
    entries = []
    if log_file.exists():
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_ATGL.NS.json"
