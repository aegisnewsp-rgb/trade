...
def get_signal():
    """Generate trading signal for TATASTEEL. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} TATASTEEL @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"TATASTEEL: No signal")
