...
def get_signal():
    """Generate trading signal for NESTLEIND. Returns (signal, price, atr)"""
    try:
        import yfinance as yf
        ticker = yf.Ticker("NESTLEIND.NS")
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} NESTLEIND @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
