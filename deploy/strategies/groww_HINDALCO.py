...
def get_signal():
    """Generate trading signal for HINDALCO. Returns (signal, price, atr)"""
    try:
        import yfinance as yf
        ticker = yf.Ticker("HINDALCO.NS")
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} HINDALCO @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"HINDALCO: No signal")
