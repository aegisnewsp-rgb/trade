...
def get_signal():
    """Generate trading signal for KOTAKBANK. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} KOTAKBANK @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"KOTAKBANK: No signal")
