...
def get_signal():
    """Generate trading signal for ICICIBANK. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} ICICIBANK @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"ICICIBANK: No signal")
