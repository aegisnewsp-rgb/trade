...
def get_signal():
    """Generate trading signal for AXISBANK. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} AXISBANK @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"AXISBANK: No signal")
