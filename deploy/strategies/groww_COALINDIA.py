...
def get_signal():
    """Generate trading signal for COALINDIA. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} COALINDIA @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"COALINDIA: No signal")
