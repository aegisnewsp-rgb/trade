...
def get_signal():
    """Generate trading signal for HDFCBANK. Returns (signal, price, atr)"""
    try:
...
def main():
    ...
    if sig:
        print(f"SIGNAL: {sig} HDFCBANK @ Rs{price:.2f} ATR:{atr:.2f}")
    else:
        print(f"HDFCBANK: No signal")
