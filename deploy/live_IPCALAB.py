#!/usr/bin/env python3
"""
Live Trading Script - IPCALAB.NS (IPCA Laboratories)
Strategy: ADX_TREND (Average Directional Index)
Position: ₹7000 | Stop Loss: 0.8% | Target: 4.0x | Daily Loss Cap: 0.3%

Research: 2026-03-22 — 20d momentum +4.3%, near 52w high (-2%), defensive pharma.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_IPCALAB.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_IPCALAB")
...
def adx_signal(ohlcv: list, params: dict) -> tuple[str, float, float]:
    ...
    SELL when -DI > +DI. Uses smoothed Directional Movement.
    """
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_IPCALAB.json"
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_IPCALAB.json"
