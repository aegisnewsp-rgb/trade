#!/usr/bin/env python3
"""
TATASTEEL.NS — VWAP Rejection SELL (Counter-Trend)
Market: INTRADAY | Exchange: NSE
Stop: +1.0% | Target: -3.0% | Time Exit: 2:30 PM IST

SELL when price < VWAP - 0.5% in downtrend confirmation.
Wait 9:45 AM IST for first 15min candle confirmation.

Run: python3 STRATEGY_TATASTEEL.py
"""
...
TGT_PCT    = 0.030    # 3.0% target (3:1 R:R)
...
def groww_auth():
    ...
    resp = requests.post(f"{API_BASE}/user/tokens", headers={
        "Content-Type": "application/json", "X-Groww-Auth-Type": "signature",
        "X-Api-Key": GROWW_KEY, "X-Request-Timestamp": ts,
        "X-Request-Signature": base64.b64encode(sig).decode()},
        json={"clientId": GROWW_KEY, "clientSecret": GROWW_SECRET, "grantType": "client_credentials"}, timeout=15)
    return resp.json().get("access_token") if resp.status_code == 200 else None

def place_bo(token, exchange, symbol, trans, qty, target, sl):
    resp = requests.post(f"{API_BASE}/orders", headers={
        "Authorization": f"Bearer {token}", "Content-Type": "application/json", "X-Api-Key": GROWW_KEY},
        json={"exchange": exchange, "symbol": symbol, "product": "INTRADAY", "orderType": "BO",
              "transactionType": trans, "quantity": qty, "targetPrice": round(target,2),
              "stopLossPrice": round(sl,2), "trailingTarget": 0.5, "trailingStopLoss": 0.3,
              "validity": "DAY"}, timeout=15)
...
def fetch_yfinance(symbol):
    ...
    return yf.Ticker(symbol).history(period="60d")

# ── Main ──────────────────────────────────────────────────────────────────
def main():
    ist = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
    print(f"\n{'='*60}")
    print(f"TATASTEEL.NS  |  {ist.strftime('%H:%M IST')}  |  VWAP Rejection SELL")
    print(f"{'='*60}")

    # Wait for 9:45 AM confirmation
    if ist.hour == 9 and ist.minute < 45:
        print("⏳ Waiting for 9:45 AM candle confirmation...")
        return
    ...
    # SELL conditions: price below VWAP, RSI not deeply oversold
    cond_bearish = price < vwap * (1 - ENTRY_VWAP_PCT)   # price rejected at VWAP
    cond_rsi_ok  = 25 < rsi < 60   # not so oversold it will bounce
    ...
    if ist.time() >= __import__("datetime").time(14, 30):
        print("2:30 PM IST — time exit"); return

    # SELL: entry = current, stop = swing high, target = below
    ...
    token = groww_auth()
    if token:
        place_bo(token, EXCHANGE, SYMBOL, "SELL", qty, target, sl)
    else:
        paper_trade("SELL", price, qty)
