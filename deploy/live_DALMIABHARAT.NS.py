#!/usr/bin/env python3
"""
Live Trading Script - DALMIABHARAT.NS
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "live_DALMIABHARAT.NS.log"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("live_DALMIABHARAT.NS")

# ── Config ────────────────────────────────────────────────────────────────────
SYMBOL         = "DALMIABHARAT.NS"
STRATEGY       = "VWAP"
POSITION       = 7000
STOP_LOSS_PCT  = 0.008
TARGET_MULT    = 4.0
...
def log_signal(signal: str, price: float, atr: float):
    log_file = LOG_DIR / "signals_DALMIABHARAT.NS.json"
    entries = []
    if log_file.exists():
        try:
...
def daily_loss_limit_hit() -> bool:
    cap_file = LOG_DIR / "daily_pnl_DALMIABHARAT.NS.json"
