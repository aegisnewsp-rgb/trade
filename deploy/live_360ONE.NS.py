...
def main():
    """Main trading loop for 360ONE"""
    import yfinance
    YFINANCE_AVAILABLE = True
    try:
        ticker = yfinance.Ticker("360ONE.NS")
        d = ticker.history(period="3mo")
        if len(d) < 30:
            print("360ONE: No data")
            return
        closes = d['Close'].tolist()
        print(f"360ONE: {len(closes)} candles, last price {closes[-1]:.2f}")
    except Exception as e:
        print(f"360ONE: Error - {e}")
